window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const tagId = "xianminglf-plugin-manager/PluginManagerSection.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "xianminglf-plugin-manager";
		...
		* (鍚嶇О / 鎻掍欢鍚嶇О / 璇︾粏) with inline skill details and a bottom-center
		* delete button in the detail panel that opens a confirm dialog. Exe export
		* lives in its own "瀵煎嚭涓篹xe" Settings section.
		...
		*/
		...
		const zh = {
			...
			columnDelete: "鍒犻櫎",
			columnSpec: "璇存槑",
			profileCatalog: "{profile} profile 鎻掍欢",
			profileEmpty: "profile 涓殏鏃犵涓夋柟鎻掍欢銆?,
			version: "鐗堟湰",
			detail: "璇︾粏",
			delete: "鍒犻櫎",
			noSkills: "璇ユ彃浠舵殏鏃?skill銆?,
			skills: "skill 鍒楄〃",
			warning: "璀﹀憡",
			confirmDelete: "纭鍒犻櫎銆寋name}銆嶆彃浠讹紵",
			confirm: "纭",
			cancel: "鍙栨秷",
			deleteFailed: "鍒犻櫎澶辫触锛歿message}",
			enable: "鍚敤鎻掍欢",
			disable: "鍋滅敤",
			enableFailed: "鎿嶄綔澶辫触锛歿message}",
			columnAction: "鎿嶄綔",
			toggleApplied: "宸插惎鐢?鍋滅敤 {name}锛屽埛鏂扮晫闈㈠悗鐢熸晥"
		};
		/** English dictionary checked against the Chinese key set. */
		const en = {
			nav: "Plugins",
			loading: "Reading plugins鈥?,
