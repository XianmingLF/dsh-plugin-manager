/* Generated by @deepseek-ai/dsh-typert-generator from the Host FaceModel — do not edit. */
import { z } from 'zod'

const dsh_xianminglf_host_plugin_manager_pluginManager_deployment_result$schema = z.object({
  'flavor': z.union([z.literal("test"), z.literal("exe")]).readonly(),
})
const dsh_xianminglf_host_plugin_manager_pluginManager_list_result$schema = z.object({
  'plugins': z.array(z.object({
  'pluginName': z.string().readonly(),
  'displayName': z.string().readonly(),
  'mainDir': z.union([z.literal(null), z.string()]).readonly(),
  'skills': z.array(z.object({
  'name': z.string().readonly(),
  'description': z.string().readonly(),
})).readonly(),
})).readonly(),
})
const dsh_xianminglf_host_plugin_manager_pluginManager_profileList_result$schema = z.object({
  'profile': z.string().readonly(),
  'plugins': z.array(z.object({
  'packageName': z.string().readonly(),
  'displayName': z.string().readonly(),
  'description': z.string().readonly(),
  'spec': z.string().readonly(),
  'version': z.string().readonly(),
  'enabled': z.boolean().readonly(),
})).readonly(),
})
const dsh_xianminglf_host_plugin_manager_pluginManager_removePlugin_parameter_0$schema = z.object({
  'pluginName': z.string().readonly(),
})
const dsh_xianminglf_host_plugin_manager_pluginManager_removePlugin_result$schema = z.object({
  'removed': z.boolean().readonly(),
  'message': z.string().readonly().optional(),
})
const dsh_xianminglf_host_plugin_manager_pluginManager_removeProfilePlugin_parameter_0$schema = z.object({
  'packageName': z.string().readonly(),
})
const dsh_xianminglf_host_plugin_manager_pluginManager_removeProfilePlugin_result$schema = z.object({
  'removed': z.boolean().readonly(),
  'message': z.string().readonly().optional(),
})
const dsh_xianminglf_host_plugin_manager_pluginManager_setProfilePluginEnabled_parameter_0$schema = z.object({
  'packageName': z.string().readonly(),
  'enabled': z.boolean().readonly(),
})
const dsh_xianminglf_host_plugin_manager_pluginManager_setProfilePluginEnabled_result$schema = z.object({
  'changed': z.boolean().readonly(),
  'message': z.string().readonly().optional(),
})

export const TYPERT_REMOTE = {
  package: '@xianminglf/plugin-manager',
  descriptors: [
    {
      id: '@xianminglf/plugin-manager#pluginManager/deployment',
      service: 'pluginManager',
      namespace: 'pluginManager',
      method: 'deployment',
      invocation: { kind: 'direct' },
      parameters: [
      ],
      result: {
        mode: 'strict',
        typeSymbol: '@xianminglf/plugin-manager/types#DeploymentInfo',
        schema: dsh_xianminglf_host_plugin_manager_pluginManager_deployment_result$schema,
      },
      sourceLocation: {"file":"packages/host/plugin-manager/src/index.ts","line":90,"column":3},
    },
    {
      id: '@xianminglf/plugin-manager#pluginManager/list',
      service: 'pluginManager',
      namespace: 'pluginManager',
      method: 'list',
      invocation: { kind: 'direct' },
      parameters: [
      ],
      result: {
        mode: 'strict',
        typeSymbol: '@xianminglf/plugin-manager/types#PluginManagerSnapshot',
        schema: dsh_xianminglf_host_plugin_manager_pluginManager_list_result$schema,
      },
      sourceLocation: {"file":"packages/host/plugin-manager/src/index.ts","line":99,"column":9},
    },
    {
      id: '@xianminglf/plugin-manager#pluginManager/profileList',
      service: 'pluginManager',
      namespace: 'pluginManager',
      method: 'profileList',
      invocation: { kind: 'direct' },
      parameters: [
      ],
      result: {
        mode: 'strict',
        typeSymbol: '@xianminglf/plugin-manager/types#ProfilePluginSnapshot',
        schema: dsh_xianminglf_host_plugin_manager_pluginManager_profileList_result$schema,
      },
      sourceLocation: {"file":"packages/host/plugin-manager/src/index.ts","line":127,"column":9},
    },
    {
      id: '@xianminglf/plugin-manager#pluginManager/removePlugin',
      service: 'pluginManager',
      namespace: 'pluginManager',
      method: 'removePlugin',
      invocation: { kind: 'direct' },
      parameters: [
        {
          name: 'request',
          wire: 'request',
          source: 'json',
          codec: {
            mode: 'strict',
            typeSymbol: '@xianminglf/plugin-manager/types#RemovePluginRequest',
            schema: dsh_xianminglf_host_plugin_manager_pluginManager_removePlugin_parameter_0$schema,
          },
        },
      ],
      result: {
        mode: 'strict',
        typeSymbol: '@xianminglf/plugin-manager/types#RemovePluginResult',
        schema: dsh_xianminglf_host_plugin_manager_pluginManager_removePlugin_result$schema,
      },
      sourceLocation: {"file":"packages/host/plugin-manager/src/index.ts","line":162,"column":9},
    },
    {
      id: '@xianminglf/plugin-manager#pluginManager/removeProfilePlugin',
      service: 'pluginManager',
      namespace: 'pluginManager',
      method: 'removeProfilePlugin',
      invocation: { kind: 'direct' },
      parameters: [
        {
          name: 'request',
          wire: 'request',
          source: 'json',
          codec: {
            mode: 'strict',
            typeSymbol: '@xianminglf/plugin-manager/types#RemoveProfilePluginRequest',
            schema: dsh_xianminglf_host_plugin_manager_pluginManager_removeProfilePlugin_parameter_0$schema,
          },
        },
      ],
      result: {
        mode: 'strict',
        typeSymbol: '@xianminglf/plugin-manager/types#RemoveProfilePluginResult',
        schema: dsh_xianminglf_host_plugin_manager_pluginManager_removeProfilePlugin_result$schema,
      },
      sourceLocation: {"file":"packages/host/plugin-manager/src/index.ts","line":191,"column":3},
    },
    {
      id: '@xianminglf/plugin-manager#pluginManager/setProfilePluginEnabled',
      service: 'pluginManager',
      namespace: 'pluginManager',
      method: 'setProfilePluginEnabled',
      invocation: { kind: 'direct' },
      parameters: [
        {
          name: 'request',
          wire: 'request',
          source: 'json',
          codec: {
            mode: 'strict',
            typeSymbol: '@xianminglf/plugin-manager/types#SetProfilePluginEnabledRequest',
            schema: dsh_xianminglf_host_plugin_manager_pluginManager_setProfilePluginEnabled_parameter_0$schema,
          },
        },
      ],
      result: {
        mode: 'strict',
        typeSymbol: '@xianminglf/plugin-manager/types#SetProfilePluginEnabledResult',
        schema: dsh_xianminglf_host_plugin_manager_pluginManager_setProfilePluginEnabled_result$schema,
      },
      sourceLocation: {"file":"packages/host/plugin-manager/src/index.ts","line":239,"column":3},
    },
  ],
}

export default TYPERT_REMOTE
